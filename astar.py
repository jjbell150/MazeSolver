...
class AStar():
	...
	def get_path(self,winning_node,nodes):	
		...
		for i in range(0,int(winning_node[1]/self.G_INCREMENT)): #Will only loop for the length of the path
			if current_node[3] != None:
				path.insert(0,current_node[3])
				for node in nodes:
					if node[0] == current_node[4]:
						current_node = node
	...
	def find_path(self):
		...
		nodes = [(self.map.start_point,0,self.get_heuristic(),None,None)] #tile index, G (cost from start), H (estimated distance), parent, direction taken
		
		actions = 0 #How many moves it took to find the end
		winning_node = None
		max_size = ((self.map.columns + self.map.rows) * self.G_INCREMENT) *2

		while winning_node == None and actions <= self.map.size:
			priority_queue = []
			
			#Finds lowest cost unexpanded node
			lowest_cost = max_size
			for node in nodes:
